...
(deeds:define-event reply (irc-event)
  ())
...
\(.+))?" (VERSION NIL DEBUG-LEVEL DESTINATION NEXT-SERVER NIL PROTOCOL-VERSION LINK-UPTIME-IN-SECONDS BACKSTREAM-SENDQ UPSTREAM-SENDQ))
    ...
    (203 RPL-TRACEUNKNOWN "[^ ]* ([^ ]+) (.+)?" (CLASS CONNECTION-ADDRESS))
    (204 RPL-TRACEOPERATOR "Oper ([^ ]+) ([^ ]+)" (CLASS NICK))
    (205 RPL-TRACEUSER "User ([^ ]+) ([^ ]+)" (CLASS NICK))
    (206 RPL-TRACESERVER "Serv
\([^ ]+) ([^S]+)S ([^C]+)C ([^
]+)
\([^@]+)@([^ ]+) (V(.+))?" (CLASS INT INT SERVER USER SERVER NIL PROTOCOL-VERSION))
    ...
    (215 RPL-STATSILINE "I ([^ ]+) \\* ([^ ]+) ([^ ]+) ([^ ]+)" (HOST HOST PORT CLASS))
    ...
    (221 RPL-UMODEIS "([^ ]+) (.+)?" (USER-MODES USER-MODE-PARAMS))
    ...
    (243 RPL-STATSOLINE "O ([^ ]+) \\* ([^ ]+) (:.*)?" (HOSTMASK NICK))
...
\([^ ]+) ([^ ]+) (:.*)
invited> has been invited by ([^ ]+)" (CHANNEL USER-BEING-INVITED USER-ISSUING-INVITE USER-ISSUING-INVITE))
    ...
    (352 RPL-WHOREPLY "([^ ]+) ([^ ]+) ([^ ]+) ([^ ]+) ([^ ]+) ([^*@+ ]+)(\\*)?([@+])? (:.*)" (CHANNEL USER HOST SERVER NICK H/G INFO))
    ...
    (437 ERR-UNAVAILRESOURCE "([^ ]+) (:.*)" (NICK/CHANNEL/SERVICE INFO))
    ...
    (443 ERR-USERONCHANNEL "([^ ]+) ([^ ]+) (:.*)?" (NICK CHANNEL))
    ...
    (641 RPL-DUMPRPL "" NIL)
    (642 RPL-EODUMP "" NIL)
    (660 RPL-TRACEROUTE-HOP "([^ ]+) ([^ ]+) (([^ ]+) ([^ ]+|\\*)? (.+))?" (TARGET HOP-COUNT NIL ADDRESS HOSTNAME USEC-PING))
    ...
    (724 RPL-TESTMARK "([^!]+)!([^@]+)@([^ ]+) ([^ ]+) ([^ ]+) (:.*)" (NICK USER HOST ? ? INFO))
    (725 RPL-TESTLINE "([^ ]+) ([^ ]+) ([^ ]+) (:.*)" (? ? ? INFO))
    (726 RPL-NOTESTLINE "([^ ]+) (:.*)" (? INFO))
    (771 RPL-XINFO "" NIL)
